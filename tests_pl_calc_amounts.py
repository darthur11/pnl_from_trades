...
def test_running_balance_calc(input_data_running_balance: pd.DataFrame):
    ...
    expected_data = pd.DataFrame(
        [
            ...
            ('USD/KZT', 'USD', 'KZT', 1, 100, 450, datetime(2020, 2, 2, 0, 0), 100, 101),
            ('USD/PHP', 'USD', 'PHP', 1, 50, 66, datetime(2020, 2, 2, 0, 0), 50, 50),
            ('USD/KZT', 'USD', 'KZT', -1, 201, 450, datetime(2020, 2, 3, 0, 0), -201, -100),
            ('USD/PHP', 'USD', 'PHP', -1, 150, 66, datetime(2020, 2, 3, 0, 0), -150, -100),
            ('USD/KZT', 'USD', 'KZT', 1, 302, 450, datetime(2020, 2, 4, 0, 0), 302, 202),
...
def test_lag_running_balance_calc(input_data_lag_running_balance: pd.DataFrame):
    ...
    expected_data = pd.DataFrame(
        [
            ('USD/KZT', 'USD', 'KZT', 1, 1, 450, datetime(2020, 2, 1, 0, 0), 1, 1, None),
            ('USD/KZT', 'USD', 'KZT', 1, 100, 450, datetime(2020, 2, 2, 0, 0), 100, 101, 1),
            ('USD/PHP', 'USD', 'PHP', 1, 50, 66, datetime(2020, 2, 2, 0, 0), 50, 50, None),
            ('USD/KZT', 'USD', 'KZT', -1, 201, 450, datetime(2020, 2, 3, 0, 0), -201, -100, 101),
            ('USD/PHP', 'USD', 'PHP', -1, 150, 66, datetime(2020, 2, 3, 0, 0), -150, -100, 50),
            ('USD/KZT', 'USD', 'KZT', 1, 302, 450, datetime(2020, 2, 4, 0, 0), 302, 202, -100),
            ('USD/PHP', 'USD', 'PHP', 1, 350, 66, datetime(2020, 2, 4, 0, 0), 350, 250, -100),
        ], columns=BASE_COLUMNS + ['amount_signed', 'running_balance', 'lag_running_balance']
    )
...
def test_amount_liquidated_calc(input_amount_liquidated_calc: pd.DataFrame):
    ...
    expected_data = pd.DataFrame(
        [
            ...
            'amount_signed', 'running_balance', 'lag_running_balance', 'amount_liquidated'
            ]
    )
